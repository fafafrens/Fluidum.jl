...
function τ_diffusion_hadron(T,μ,x::Heavy_Quark) where {M}
    ...
    for i in x.hadron_list.particle_list
        ...
        tauq = (q*n*(2*π*x.DsT)*m^3/T^3/(96*π*T)*(2*b1 - 3*b3 +b5)/b2)/fmGeV;
        tauq += tauq; #sum((q*n*(2*π*x.DsT)*m^3/T^3/(96*π*T)*(2*b1 - 3*b3 +b5)/b2)/fmGeV); #
    end
