...
function τ_diffusion_hadron(T,x::HQdiffusion{M}, y::Heavy_Quark) where {M}
    
    for i in y.hadron_list.particle_list
        m = i.mass
        b2 = besselkx(2,m/T)*exp(-m/T)
        b1 = besselkx(1,m/T)*exp(-m/T)
        b3 = b1+4/(m/T)*b2  
        ...
        b5 = b3+8/(m/T)*b4
        q = i.Nc
        n = thermodynamic(T,y.hadron_list).density[i]
        tauq = sum((q*n*(2*π*x.DsT)*m^3/T^3/(96*π*T)*(2*b1 - 3*b3 +b5)/b2)/fmGeV); #
    end
    #norm = sum(y.q*y.n in (y.hadron_list.particle_list))
   
  return ((2*π*x.DsT) *m^3/T^3/(96*π*T)*(2*b1 - 3*b3 +b5)/b2)/fmGeV; #
  
end

function τ_diffusion_hadron(y::Heavy_Quark) where {M}
    ...
    for i in y.hadron_list.particle_list
        ...
        print("the particle", name, "with mass ", m, "have charge", q)
...
τ_diffusion_hadron(y::Heavy_Quark)
