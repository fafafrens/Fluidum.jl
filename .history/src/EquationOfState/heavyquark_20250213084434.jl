...
function free_hadron(m,deg,T,fug)
...
function norm(T,μ,x::Heavy_Quark)
    for i in x.hadron_list.particle_list
        m = i.Mass
        deg = i.Degeneracy
        q = i.Nc + i.Nac
        n = free_hadron(m,deg,T,μ)[1]
        norm += q^2*n
    end 
    return norm/fmGeV;
end

function τ_diffusion_hadron(T,μ,x::Heavy_Quark,y::HQdiffusion{M}) 
    ...
    for i in x.hadron_list.particle_list
        ...
        q = i.Nc + i.Nac
        exp = exp(q*μ/T)
        #tauq += (q^2*n*(2*π*x.DsT)*m^3/T^3/(96*π*T)*(2*b1 - 3*b3 +b5)/b2)/fmGeV;
        tauq += ((2*π*y.DsT)*exp*q^2*m^5/(128*π^3*T^3)*(2*b1 - 3*b3 +b5))/fmGeV; 
    end
       
  return tauq/norm(T,μ,x); #
