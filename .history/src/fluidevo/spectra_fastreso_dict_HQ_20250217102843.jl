...
@inline function cilindrical_thermal_spectra(q,r,t,dra,dta,T,ν,μ,K1eq,K2eq,K1diff,K2diff)
...
@inline function _pointwise_spectra(pt,alpha,x::A,phi::B,part::particle_attribute{S,R,U,V};decays, ccbar = 2.76) where {A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    ...
    cilindrical_thermal_spectra(q,r,t,dra,dta,T,ν,μ,K1eq,K2eq,K1diff,K2diff)*deg*fact
end
