...
function save_all_particles(;T=1100, type = "thermal")
    kernel_path = "C:/Data/Università/Germania/PhD/Kernels" #personal path to the kernels folder
    light_path = string(kernel_path,"/light_kernels/")
    save_kernels(;T=T,type = type, file_path=light_path, MC = true)

    heavy_path = string(kernel_path,"/heavy_kernels/")
    save_kernels_diff(;T=T,type = type, file_path=heavy_path, MC = true)
end 
