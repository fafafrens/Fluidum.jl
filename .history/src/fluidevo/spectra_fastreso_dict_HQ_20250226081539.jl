...
@inline function cilindrical_thermal_spectra(q,r,t,dra,dta,T,ν,μ,norm,K1eq,K2eq,K1diff,K2diff; delta_f)
...
@inline function _pointwise_spectra(pt,alpha,x::A,phi::B,part::particle_attribute{S,R,U,V},norm;decays, ccbar = 2.76,delta_f) where {A<:SplineInterp,B<:SplineInterp,S,R,U,V}
...
function spectra(pt::C,fo::FreezeOutResult{A,B},part::particle_attribute{S,R,U,V};rtol=1000*sqrt(eps()),decays=true,ccbar = 2.76) where {C<:Number,A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    ...
    quadgk(alpha->_pointwise_spectra(pt,alpha,x,phi,part,norm;decays,ccbar,delta_f),lb...,rb...,rtol=rtol)
...
function spectra(fo::FreezeOutResult{A,B},part::particle_attribute{S,R,U,V};pt_min=0.,pt_max=10.0,step=100,rtol=1000*sqrt(eps()),decays=true,rightbound=(100,),ccbar = 2.76,delta_f=false) where {A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    ...
    [quadgk(alpha->_pointwise_spectra(pt,alpha,x,phi,part,norm;decays,ccbar,delta_f),lb...,rb...;segbuf=buff,rtol=rtol) for pt in range(pt_min,pt_max,step) ] 
...
function spectra(fo::FreezeOutResult{A,B},part::particle_attribute{S,R,U,V}, pt_range::Vector{Float64};rtol=1000*sqrt(eps()),decays=true,rightbound=(100,),ccbar = 2.76) where {A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    ...
    [quadgk(alpha->_pointwise_spectra(pt,alpha,x,phi,part,norm;decays,ccbar),lb...,rb...;segbuf=buff,rtol=rtol) for pt in pt_range ] 
...
function multiplicity(fo::FreezeOutResult{A,B},part::particle_attribute{S,R,U,V};rtol=1000*sqrt(eps()),decays=true, delta_f = true, rightbound=100,pt_min=0.,pt_max=10.0) where {A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    x,phi=fo
    lb=leftbounds(x)
    rb=rightbounds(x)
    rb = min.(rb,(rightbound,))
    hcubature( b ->2.0*π *_pointwise_spectra(b[1],b[2],x,phi,part,norm;decays,delta_f)*b[1],(pt_min,lb...),(pt_max,rb...);rtol=rtol)
