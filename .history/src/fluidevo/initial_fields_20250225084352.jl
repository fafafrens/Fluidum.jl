...
function fug(temperature_profile, nhard_profile, r, eos;  rdrop = 12)       
    if r<=rdrop
        return  log(nhard_profile(r)/(thermodynamic(temperature_profile(r),0.0,eos).pressure))
        else return log(nhard_profile(rdrop)/(thermodynamic(temperature_profile(rdrop),0.0,eos).pressure))
    end            
end
...
function initialize_fields(x::TabulatedTrento{A,B}, y::TabulatedTrento{C,D}, cent1::Integer, cent2::Integer, list; tau0 = 0.4, gridpoints=500,rmax=30,norm_temp=1, norm_coll=1, exp_tail = true) where {A,B,C,D}
    ...
    fugacity(r) = fug(temperature_profile, nhard_profile, r, eos)
    
    #this is the number of c+cbar 
    @show ccbar
