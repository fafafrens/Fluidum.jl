...
@inline function cilindrical_thermal_spectra(q,r,t,dra,dta,T,ν,μ,eos,norm,K1eq,K2eq,K1diff,K2diff; delta_f)
...
@inline function _pointwise_spectra(pt,alpha,x::A,phi::B,part::particle_attribute{S,R,U,V},eos, norm;decays, ccbar = 2.76,delta_f) where {A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    ...
    cilindrical_thermal_spectra(q,r,t,dra,dta,T,ν,μ,eos,norm,K1eq,K2eq,K1diff,K2diff;delta_f=delta_f)*deg*fact
end
...
function spectra(pt::C,fo::FreezeOutResult{A,B},part::particle_attribute{S,R,U,V};rtol=1000*sqrt(eps()),decays=true,ccbar = 2.76) where {C<:Number,A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    ...
    quadgk(alpha->_pointwise_spectra(pt,alpha,x,phi,part,eos,norm;decays,ccbar),lb...,rb...,rtol=rtol)
