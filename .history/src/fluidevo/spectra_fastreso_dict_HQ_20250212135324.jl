...
@inline function cilindrical_thermal_spectra(m,pt,r,t,dra,dta,ur,T,pi_phi,pi_eta,pi_b,μ,ν,K1eq,K2eq,K1diff,K2diff)
    ...
    result= (r_factor*(K1eq+K1diff*ν/norm)+t_factor*(K2eq+K2diff*ν/norm))*exp(μ)
...
@inline function _pointwise_spectra(pt,alpha,x::A,phi::B,part::particle_attribute{S,R,U,V};decays, ccbar = 2.76) where {A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    ...
    if part.name == "jp3096zer"
        fact = 1
        μ = 2μ #J/Psi ha μ = 2 perche ha 2 charm (sto facendo fugacity*charm_charge) 
    end 
    if(decays==true)
    ...
    cilindrical_thermal_spectra(pt,r,t,dra,dta,ur,T,pi_phi,pi_eta,pi_b,μ,ν,K1eq,K2eq,K1diff,K2diff,part)*deg*fact
end
