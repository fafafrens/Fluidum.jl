...
function get_kernels(;type = "thermal", file_path=pwd()*"/src/FastReso_OC_kernels/", MC = false)
...
function save_kernels(;type = "thermal", file_path=pwd()*"/src/FastReso_OC_kernels/", MC = false)    
    particle_name, kernel_path = get_kernels(; type = type, file_path=file_path, MC = MC)
    
    for name in particle_name
        a = readdlm(kernel_path[Symbol(name)][i], Float64,skipstart=1) #same for all
...
function save_kernels_diff(;type = "thermal", file_path=pwd()*"/src/FastReso_OC_kernels/", MC = false)    
    particle_name, kernel_path = get_kernels(; type = type, file_path=file_path, MC = MC)
    ...
    for name in particle_name
        ...
        ext = extrapolate.(itp, Ref(Flat()))    
        #@show ext
        interp_kernel=kernels_diff(ext...) #performing "unpaking": taking all the elements of ext, and using them to fill kernels 
        @save file_path*name*"_"*type*".jld" interp_kernel
    end    
end


function save_all_particles(;type = "thermal")
    save_kernels(;type = type, file_path="$(@__DIR__)/Kernels_folder/FastReso_kernels/", MC = true)
end 


#uncomment only to save new kernels
#save_all_particles(type = "thermal")
#save_all_particles(type = "total")
