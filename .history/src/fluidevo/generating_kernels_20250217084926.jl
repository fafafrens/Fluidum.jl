...
function save_all_particles(;T=1100, type = "thermal")
    kernel_path = "C:/Data/Università/Germania/Tesi_mag/solver/src/FieldsEvolution/src/Kernels_folder" #personal path to the kernels folder
    light_path = string(kernel_path,"/FastReso_kernels/")
    save_kernels(;T=T,type = type, file_path=light_path, MC = true)

    heavy_path = string(kernel_path,"/FastReso_kernels/")
    light_path = string(kernel_path,"/FastReso_OC_kernels/")
    save_kernels_diff(;T=T,type = type, file_path=heavy_path, MC = false)
end 


#uncomment only to save new kernels
save_all_particles(;T=1100, type = "thermal")
save_all_particles(;T=1100, type = "total")
