...
@inline function _pointwise_spectra(pt,alpha,x::A,phi::B,part::particle_attribute{S,R,U,V},normm;decays, ccbar = 2.76,delta_f) where {A<:SplineInterp,B<:SplineInterp,S,R,U,V}
...
function spectra(pt::C,fo::FreezeOutResult{A,B},part::particle_attribute{S,R,U,V};rtol=1000*sqrt(eps()),decays=true,ccbar = 2.76) where {C<:Number,A<:SplineInterp,B<:SplineInterp,S,R,U,V}
    ...
    quadgk(alpha->_pointwise_spectra(pt,alpha,x,phi,part,normm;decays,ccbar,delta_f),lb...,rb...,rtol=rtol)
